...
void VMC_calc_1_right (vmc_leg_t *vmc, INS_t *ins, float dt)//计算theta和d_theta给lqr用，同时也计算腿长L0
{
	...
	PitchR = chassis_move . myPithR;
	PithGyroR = chassis_move . myPithGyroR;
...
void VMC_calc_1_left (vmc_leg_t *vmc, INS_t *ins, float dt)//计算theta和d_theta给lqr用，同时也计算腿长L0
{
	...
	PitchL = chassis_move . myPithL;
	PithGyroL =chassis_move . myPithGyroL;
...
float LegForceCalc (vmc_leg_t *vmc, float T1, float T2)
{
	static float det = 0;
	...
	vmc -> mea_F = vmc -> inv_j11 * T1 + vmc -> inv_j12 * T2;
//	vmc -> mea_Tp = vmc -> inv_j21 * T1 + vmc -> inv_j22 * T2;
